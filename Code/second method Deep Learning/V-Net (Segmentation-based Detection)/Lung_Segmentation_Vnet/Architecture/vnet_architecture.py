...
'''
from Architecture.layer import (conv3d, deconv3d, normalizationlayer, crop_and_concat, resnet_Add,
...
class Vnet3dModule(object):
    ...
    def __init__(self, image_height, image_width, image_depth, channels=1, costname=("dice coefficient",),
        ...
        self.X = tf.compat.v1.placeholder("float", shape=[None, self.image_depth, self.image_height, self.image_width,
                                                self.channels])
        self.Y_gt = tf.compat.v1.placeholder("float", shape=[None, self.image_depth, self.image_height, self.image_width,
                                                   self.channels])
        self.lr = tf.compat.v1.placeholder('float')
        self.phase = tf.compat.v1.placeholder(tf.bool)
        self.drop = tf.compat.v1.placeholder('float')
    ...
    def train(self, train_images, train_lanbels, model_path, logs_path, learning_rate,
        ...
        train_op = tf.train.AdamOptimizer(self.lr).minimize(self.cost , var_list=None)
